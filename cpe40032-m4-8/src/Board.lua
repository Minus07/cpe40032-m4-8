...
function Board:init(x, y, level)
    ...
    --choose random color based on level
    self.diffColorsCount = math.min(10, level + 5)
    self.diffColors = self:getRandomColors(self.diffColorsCount)

    --generate random variety of tiles based on level
    self.maxVariety = math.min(6, math.ceil(level / 2))
        ...
        for x = 2, 8 do
            ...
            if self.tiles[y][x].color == colorToMatch then
                ...
                if matchNum >= 3 then
                    ...
                    -- first check for shiny tile in match
    ...
    for x = 1, 8 do
        for y = 8, 1, -1 do
            ...
            if not tile then
                ...
                --creates new tween to return for tile to fall down
                tweens[tile] = {
                    y = (tile.gridY - 1) * 32
                }
            end
...
--checks if there's any possible matches in the board
