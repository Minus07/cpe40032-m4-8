...
--swapBackOnNoMatch swaps back tiles if there is no match
...
function PlayState:calculateMatches(calculatedMatches)
    ...
    if matches then
        ...
        -- tween new tiles that fall from the ceiling to fill new gaps that exist
        ...
        if not self.board:anyMatches() then
            ...
            --start a transition of the text label to the center of the screen
            Timer.tween(0.5, {
                [self] = {noMatchLabelY = VIRTUAL_HEIGHT / 2 - 8}
            })

            --after that pause for 1 sec with timer.after
            :finish(function()
                Timer.after(2, function()
                    -- then animate the label going down
                    Timer.tween(0.5, {
                        [self] = {noMatchLabelY = VIRTUAL_HEIGHT + 30}
                    })
                    
                    --once that's complete, reload the board
                    :finish(function()
...
function PlayState:render()
    ...
    --if board has no matches render a text and background
    if self.noMatches then
        love.graphics.setColor(95, 205, 228, 300)
